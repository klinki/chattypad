...
only when the feature specification does not call for them; every user story
MUST still include explicit validation work such as quickstart verification or
...
- [ ] T018 [US1] Validate the user story via [quickstart step/manual check/test] in [path]
...
- [ ] T025 [US2] Validate the user story via [quickstart step/manual check/test] in [path]
...
- [ ] T031 [US3] Validate the user story via [quickstart step/manual check/test] in [path]
...
- [ ] TXXX Update agent context or delivery guidance in [path]
...
- Every story MUST include an explicit validation task even when no automated
  tests are requested
